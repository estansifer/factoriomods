...
local function resource_autoplace_settings(params)
    local scale = params.scale or 1 -- ems
    local starting_density = params.starting_density or 1 -- ems
    local regular_patch_offset = params.regular_patch_offset -- ems
    local starting_patch_offset = params.starting_patch_offset -- ems
    local x_scale = noise.var('x') * tne(scale) -- ems
    local y_scale = noise.var('y') * tne(scale) -- ems
  ...
  local distance = noise.var("distance") * scale -- ems
  ...
  local double_density_distance = 1300 * scale -- ems
  ...
  local starting_amount = 40000 * base_density * starting_frequency_multiplier * size_multiplier * starting_density -- ems
  ...
  local regular_spots = tne{
    ...
    arguments =
    {
      ...
      skip_offset = tne(regular_patch_offset), -- ems
  ...
  if params.has_starting_area_placement == true then
    starting_patch_set_index = tne(starting_patch_offset) -- ems
  end
